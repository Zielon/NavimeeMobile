package org.pl.android.drively.util;

public class FirebasePaths {
    public static final String USERS = "USERS";
    public static final String AVATARS = "AVATARS";
    public static final String MESSAGES_PRIVATE = "MESSAGES_PRIVATE";
    public static final String MESSAGES_GROUPS = "MESSAGES_GROUPS";
    public static final String HOTSPOT = "HOTSPOT";
    public static final String HOTSPOT_CURRENT = "HOTSPOT_CURRENT";
    public static final String FEEDBACK = "FEEDBACK";
    public static final String NOTIFICATIONS = "NOTIFICATIONS";
    public static final String GROUP = "GROUP";
    public static final String ROOM_DETAILS = "ROOM_DETAILS";
    public static final String MEMBERS = "MEMBERS";
    public static final String FRIENDS = "FRIENDS";
    public static final String USER_LOCATION = "USER_LOCATION";
    public static final String AVAILABLE_CITIES = "AVAILABLE_CITIES";
    public static final String AVAILABLE_CITIES_NATIVE = "AVAILABLE_CITIES_NATIVE";
    public static final String NOT_AVAILABLE_CITIES = "NOT_AVAILABLE_CITIES";
    public static final String BY_CITY = "BY_CITY";
    public static final String UBER = "UBER";
}
